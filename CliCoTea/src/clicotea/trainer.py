...
def train_token_alignment(
    ...
    loss_fn = F.mse_loss
    n_saved = args.n_saved # 2
    device = args.device
    epochs = args.epoch 
    output_dir = args.output_dir
    max_grad_norm = args.max_grad_norm # 1.2
    gradient_accumulation_steps = args.gradient_accumulation_steps # 1
    exp_name = args.exp_name if args.exp_name else None # "token-align"
    run_name = args.run_name if args.run_name else None # ""
    log_every_iters = args.log_every_iters # 100

    logger.info("Start training")

    # set an optimizer algorithm
    optimizer = set_optimizer(args, model) # adam
    # with a scheduler
    lr_scheduler = set_scheduler(args, optimizer, logger) # constant
