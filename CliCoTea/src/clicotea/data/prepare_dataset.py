...
def sentence_to_ids(sentence: str, tokenizer: AutoTokenizer):
    sent = sentence.strip().split() # 句子->单词->token->token id->标签扁平化并且通过编号返回组的关系
    ...
    tensor_ids = tokenizer.prepare_for_model( #在这里添加了首项101和末项102
...
def get_word_pairs(src, tgt, model, tokenizer, device):
    ...
    with torch.no_grad():
        ...
        out_tgt = model(ids_tgt.unsqueeze(0).to(device), output_hidden_states=True)[2][
            align_layer
        ][0, 1:-1] #模型添加一个第0项处理后，取hidden state中的第align_layer层，然后剔除第一行和最后一行
    ...
    return align_words #返回一组tuple里面装着对应align words的序号


def load_sentences(annotations: List, item: str = "sentence"):
    ## 一个简单函数，返回文件中每条数据的caption部分，并flatten可能的2d list
...
def generate_token_pairs(
    ...
    for i in tqdm(range(n_sentences)):
        ...
        dataset_word_pairs.append(list(word_pairs)) # 用list形式append
