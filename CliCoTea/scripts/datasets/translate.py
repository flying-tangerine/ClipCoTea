...
def translate_file(ann, total_data, dest, keys=["sentence"], max_iteration=1000):
    translator = Translator(
        service_urls=["translate.googleapis.com"]
    )  # , timeout=httpx._config.Timeout(60))

    translated_ann, skipped_ann = [], []
    for i, item in tqdm(enumerate(ann), total=total_data): # total=len(ann)
        iteration = 0
        stop = False
...
if __name__ == "__main__":
    ...
    with jsonlines.open(args.input_file) as ann: ##
        total_data = sum(1 for _ in ann)
    ann = jsonlines.open(args.input_file)
    translated_ann, skipped_ann = translate_file(
        ann, total_data, args.lang, keys=args.field, max_iteration=args.max_iterations
    )
